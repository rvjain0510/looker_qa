view: dim_houshld_accnt_historical_pdt {
    derived_table:
    {
      sql: select
            ...
            JOIN `@{bq_project_id}.@{bq_dataset_name}.FACT_REL_REGSTR_REPRSNTTV_ACCNT` rep on accnt.MD_HASH_NAT_KEYS = rep.NK_HK_DIM_ACCNT
            left JOIN `@{bq_project_id}.@{bq_dataset_name}.FACT_REL_ACCNT_PARTY_MV` ap on ap.NK_HK_DIM_ACCNT = accnt.MD_HASH_NAT_KEYS and ap.MD_ACTIVE
            left JOIN `@{bq_project_id}.@{bq_dataset_name}.DIM_PARTY_MV` party on party.MD_HASH_NAT_KEYS = ap.NK_HK_DIM_PARTY and party.MD_ACTIVE
            GROUP BY 1,2,7;;
