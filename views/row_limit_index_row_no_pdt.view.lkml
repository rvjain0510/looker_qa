# This PDT is created for AX360 to provide rownumber as a index column for user - account - repcd - usertype combination.
view: row_limit_index_row_no_pdt {
  derived_table: {
    ...
    dim_houshld_accnt_pdt.nk_hk_dim_accnt AS nk_hk_dim_account,
    dim_houshld_accnt_pdt.nk_hk_dim_regstr_reprsnttv AS nk_hk_dim_regstr_reprsnttv,
    ROW_NUMBER() OVER(order by dim_user_unnested.USERID,dim_user_unnested.REP_CODE,dim_user_unnested.USER_TYPE,dim_houshld_accnt_pdt.nk_hk_dim_accnt) AS index_row_no
  ...
  dimension: nk_hk_dim_account {
    type: string
    sql: ${TABLE}.nk_hk_dim_account ;;
  }
