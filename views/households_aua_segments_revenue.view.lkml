view: households_aua_segments_revenue {
#This view is created to build age and income segments for households.

  derived_table: {

    explore_source: ax360_dashboards_hh_revenue {
      column: houshld_id { field: houshlds.houshld_id }
      column: houshld_houshld_name { field: houshlds.houshld_name }
      column: total_aua_12_month { field: total_aua_last_12_months_pdt.total_aua_12_months_for_clients }

      #1168403-changes-for-j-and-closed-accounts: To eliminate symmetric aggregation
      column: total_aua { field: total_aua_last_12_months_pdt.total_aua_current_month_for_clients_wo_coalesce }
  ...
  measure: households_aua_12_month {
    label: "total_market_value_cad" #this is localized
    type: sum
    sql: ${TABLE}.total_aua_12_month ;;
    value_format_name: decimal_0
    html:
        {% if value < 0 %}
        <span style="color:#B5252A;">{{rendered_value}}</span>
